...
int main(int argc, char** argv){
  ...
  while (ros::ok())
  {
    ...
    if(hasAllPoints){
      ...
      //Creating a vector that consists of points for planting. The order of these point will form the route
      ...
      // Placing a marker for each of the points 
      for (int i = 0; i < grid.size()/2; i++)
        {
        updateMarker(marker_pub, marker, seedlingsPoints[i], i + 10);
        cout << seedlingsPoints[i].x << seedlingsPoints[i].y << "\n";
        sleep(1);
        }
      
      // Now that all points are created, we can draw lines between the corners. //
  ...
  //Moves to each point, and makes a sound, performing a "planting sequence"
  for (int i = 0; i < gridPointsAmount; i++){
    //Writes where it is supposed to go. Good for debugging and generel information about the process
...
void getPointerPose(const geometry_msgs::PointStamped point){
  ...
  //Saving the points and when 4 points are made it will update hasAllPoints so the planting can begin
